...
---
#### 💡 Supplemental: 동기, 비동기, 병렬
|구분|물리적 상태|하드웨어적 비용|실무적 교훈|
|--|--|--|--|
|동기 (Sync)|1인 1역 (순차 처리)|없음 (기다림이 비용)|단순하지만 대규모 처리 불가|
|비동기 (Async)|1인 다역 (시분할)|Context Switch 발생|대기 시간을 연산으로 채움|
|병렬 (Parallel)|다인 다역 (멀티코어)|자원 분배/동기화 비용|물리적 속도 향상의 유일한 길|

|용어	|핵심 정의	|실무적 비유	|하드웨어적 비용|
|--|--|--|--|
|Context	|레지스터의 현재 비트 상태|	학자의 머릿속 생각과 메모|	없음 (상태 그 자체)|
|Context Switch	|문맥 보관 및 복구 작업	|하던 일 적어두고 다른 책 펴기	|매우 높음 (Memory Latency 발생)|
---
#### 💡 Ex : 매일 밤 반복 작업 최적화
